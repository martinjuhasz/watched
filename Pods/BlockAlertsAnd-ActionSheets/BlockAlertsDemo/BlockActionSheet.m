...
+ (void)initialize
{
    if (self == [BlockActionSheet class])
    {
        ...
        titleFont = [[UIFont systemFontOfSize:17] retain];
        buttonFont = [[UIFont boldSystemFontOfSize:17] retain];
    }
}
...
- (void)showInView:(UIView *)view
{
    ...
    for (NSArray *block in _blocks)
    {
        ...
        UIImage *imageActive = [UIImage imageNamed:[NSString stringWithFormat:@"action-%@-button-selected.png", color]];
        imageActive = [imageActive stretchableImageWithLeftCapWidth:(int)(imageActive.size.width)>>1 topCapHeight:0];
